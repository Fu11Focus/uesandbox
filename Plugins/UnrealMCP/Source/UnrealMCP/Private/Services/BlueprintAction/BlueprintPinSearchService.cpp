...
)
{
    ...
    if (TargetClass)
    {
        for (TFieldIterator<FProperty> PropIt(TargetClass, EFieldIteratorFlags::IncludeSuper); PropIt; ++PropIt)
        {
            ...
            FString PropPinType = Property->GetCPPType();
            FString Category = TEXT("Native Property");
            FString Keywords = FString::Printf(TEXT("property variable %s %s native"), *PropName, *PropPinType);
            FString Tooltip = FString::Printf(TEXT("Access the %s property on %s"), *PropName, *TargetClass->GetName());
            // Apply search filter
            if (!SearchFilter.IsEmpty() && !(PropName.ToLower().Contains(SearchFilter.ToLower()) || PropPinType.ToLower().Contains(SearchFilter.ToLower()) || Keywords.ToLower().Contains(SearchFilter.ToLower())))
            {
                continue;
            }
            // Getter node
            {
                ...
                GetterObj->SetStringField(TEXT("pin_type"), PropPinType);
            ...
            if (Property->HasMetaData(TEXT("BlueprintReadWrite")) && !Property->HasMetaData(TEXT("BlueprintReadOnly")) && !Property->HasAnyPropertyFlags(CPF_ConstParm))
            {
                ...
                SetterObj->SetStringField(TEXT("pin_type"), PropPinType);
